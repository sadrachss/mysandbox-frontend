...
export const adminService = {
  ...
  unbanUser: async (id: string) => {
    const { data } = await api.post(`/api/admin/users/${id}/unban`);
    return data;
  },
  ...
  deleteLink: async (id: string) => {
    const { data } = await api.delete(`/api/admin/links/${id}`);
    return data;
  },

  // Analytics
  getAnalytics: async (days?: number) => {
    const { data } = await api.get('/api/admin/analytics', { params: { days } });
    return data;
  },
